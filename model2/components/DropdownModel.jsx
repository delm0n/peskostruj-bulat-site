...
function Dropdown({ classItem, payload, mainDops, selectDops }) {
  ...
  useEffect(() => {
    setHeightList(document.querySelector(`.${classItem} .dropdown__list`));
  }, []);

  return (
    <Transition in={dropdownActive} timeout={200}>
      {(state) => (
        <div className={`dropdown ${classItem} ${state}`}>
          <div
            onClick={() => setDropdownActive(!dropdownActive)}
            className={`dropdown__hero ${state}`}
          >
            {selectDops.map((item) => (
          ...
          <ul
            ...
            className={`dropdown__list ${state}`}
          >
            {selectDops.map((item) => (
              <li
                onClick={() => dropdownСhoose(item.defaultModelId)}
                key={item.id}
                className={`dropdown__item ${item.selected ? "active" : ""}`}
              >
                {item.name}
              </li>
            ))}
